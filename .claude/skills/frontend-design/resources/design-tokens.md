...
/* Display - Headlines */
--font-display:
  'Cabinet Grotesk', 'Plus Jakarta Sans', -apple-system, BlinkMacSystemFont, sans-serif;
...
```css
.btn-primary {
  @apply rounded-lg bg-[#0183FF] px-6 py-3 font-medium text-white;
  @apply transition-colors hover:bg-[#0170E0];
...
```css
.btn-secondary {
  @apply rounded-lg bg-white px-6 py-3 font-medium text-[#1D2125];
...
```css
.btn-ghost {
  @apply bg-transparent px-6 py-3 font-medium text-[#0183FF];
  @apply rounded-lg transition-colors hover:bg-[#EBF5FF];
}
```
